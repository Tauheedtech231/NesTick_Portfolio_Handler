...
export default function UniqueDiagram({ 
  ...
  const [showCard, setShowCard] = useState(true); // 🔥 Default true
  const [isHovering, setIsHovering] = useState(false);
  const [cardAnimating, setCardAnimating] = useState(true); // 🔥 Default true
  ...
  // 🎯 Auto hide after description complete (only when not hovering)
  useEffect(() => {
    ...
    // 🔥 Only auto-hide when NOT hovering AND NOT userInteracted
    if (progress === 100 && !isHovering && !userInteracted) {
      hideTimerRef.current = setTimeout(() => {
        setCardAnimating(false);
        setTimeout(() => {
          // Show next feature after brief pause
          const nextIdx = (activeIdx + 1) % features.length;
          onHover(nextIdx);
  ...
  }, [progress, isHovering, userInteracted, activeIdx, features.length, onHover]);

  // Card animation for hover
  useEffect(() => {
    if (isHovering) {
      setShowCard(true);
      setCardAnimating(true);
    } else {
      // Don't hide card, just keep showing with auto-loop
      setShowCard(true);
      setCardAnimating(true);
    }
  }, [isHovering]);
      ...
      <div className="flex items-center justify-center" style={{ gap: '0px', minWidth: '550px' }}>
        ...
        <div ref={diagramContainerRef} className="relative w-[500px] h-[500px] flex-shrink-0">
          ...
          >
            ...
            {/* 🎯 DOTTED CURVED ARROW - Always visible when card is shown */}
            {showCard && (
              <>
                <path
                  d={arrowPath}
        ...
        {/* 🎯 DESCRIPTION CARD - Always visible on right side */}
        <div 
